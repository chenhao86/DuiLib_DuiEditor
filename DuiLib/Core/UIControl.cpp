...
namespace DuiLib {
	...
	CDuiString CControlUI::GetText() const
	{
		...
		{
			CDuiString s = pkg->GetText(GetResourceIDEx());
	...
	void CControlUI::SetResourceID(int resid)
	{
		...
		SetResourceID(duiStr);
	}

	void CControlUI::SetResourceID(CDuiString resid)
	{
		if (m_strResourceId == resid)
		{
			return;
		}
		m_strResourceId = resid;
		Invalidate();
	}

	DuiLib::CDuiString CControlUI::GetResourceIDEx() const
	{
		return m_strResourceId;
	}
	...
	CDuiString CControlUI::GetToolTip() const
	{
		...
		if (pkg && !GetResourceIDEx().IsEmpty())
		{
			CDuiString s = pkg->GetToolTip(GetResourceIDEx());
			if (!s.IsEmpty()) return s;
		}
	...
	void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcsicmp(pstrName, _T("resourceid")) == 0) SetResourceID(pstrValue);
